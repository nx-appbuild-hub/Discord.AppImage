...
all:
	...
	mkdir --parents $(PWD)/build/AppDir/discord

	wget --output-document=$(PWD)/build/build.deb https://dl.discordapp.net/apps/linux/0.0.12/discord-0.0.12.deb
	...
	cp --force --recursive $(PWD)/build/usr/share/discord/* $(PWD)/build/AppDir/discord
	...
	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/AppDir $(PWD)/Discord.AppImage
